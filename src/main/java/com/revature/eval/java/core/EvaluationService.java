...
*/
	...
	public String acronym(String phrase) {
		...
		int n = 0;
		phrase = phrase.trim();
		if(phrase.length()>0) {n = 1;}
		for(int i = 0; i < phrase.length();i++) {
			if(phrase.charAt(i) == ' '|| phrase.charAt(i) == '-') {n++;}
		}
		char[] ac = new char[n];
		String acstr = "";
		if(phrase.length()>0) {
			...
			for(int i = 0; i < phrase.length();i++) {
				if(i == 0 || phrase.charAt(i-1) == ' '|| phrase.charAt(i-1) == '-') {
					ac[ind] = phrase.charAt(i);
					ind++;
				}
			}
		}
		for(char s:ac) {acstr += s;}
		return acstr.toUpperCase();
	}
	...
	static class Triangle {
		...
		public boolean isEquilateral() {
			// TODO Write an implementation for this method declaration
			boolean r = false;
			if((this.sideOne == this.sideTwo) && this.sideTwo == this.sideThree) {r = true;}
			return r;
		}
		...
		public boolean isIsosceles() {
			...
			int n = 0;
			boolean r = false;
			if(this.sideOne == this.sideTwo) {n++;}
			if(this.sideThree == this.sideTwo) {n++;}
			if(this.sideThree == this.sideOne) {n++;}
			if(n>0){r = true;}
			return r;
		}
    ...
    string = string.trim();
		...
		for(int i = 0; i < string.length();i++) {
			pnraw[i] = Character.getNumericValue(string.charAt(i));
		}
		String pn = "";
		boolean first = true;
		//filters out bad integer conversions
		for(int n:pnraw) {
			if(n !=-1) {
				if(!(n == 1 && first)) {
					pn+=n;
		...
		//if illegal characters were used in place of digits
		if(pn.length() !=10) {throw new IllegalArgumentException();}
		return pn;
	}
